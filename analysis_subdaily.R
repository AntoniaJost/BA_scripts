...
# first the obs
obs.rot.1d = sl.trajectory.remaptime(oldtime = obs$res.list[[1]]$data$DaysLeadTime, newtime = 1,
                                     oldlat = obs.rot$lat, oldlon = obs.rot$lon)
obs.rot$lon = obs.rot$lon - obs.rot.1d$Lon
# now the fcst
...
plot(x=fcst.lin$res.list[[1]]$data$DaysLeadTime, y=fcst.lin.eval$res.list[[1]]$ens.mean.relspeed, xlab="days lead time", ylab = "relative distance",
