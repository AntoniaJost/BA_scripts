...
if(Sys.info() ["user"] == "anjost001") {
  ...
  save_dir = "error_quantification" # name of (new) folder where all data is going to be stored
...
# input
# hier als naechstes noch optimieren, dass Input individuell und Daten automatisch erzeugt werden
...
daysLeadTime = fcst$res.list[[1]]$data$DaysLeadTime # extract Leadtime
...
subdiv_dataset = function(dataset) { 
  ...
  for (i.num in 1:length(integers)) { # loop over integers (=every day)
    ...
    startIdx = which(floor(daysLeadTime) == currentNumber) # index for beginn of dataset
    ...
    if(i.num == length(integers)-1) { # if last set (day 9 to 10) add last single value of 10.000 to dataset of day 9
      ...
      teilDatensatz = dataset$res.list[[1]]$data[startIdx[1]:startIdx[length(startIdx)],] 
      teilDatensaetze[[i.num]] = teilDatensatz 
      break # stop loop
    }
    
    teilDatensatz = dataset$res.list[[1]]$data[startIdx[1]:startIdx[length(startIdx)],] # extract
