# function to calculate mean daily error
...
calc.mean = function(colmean) {
  for(i.day in 1:(length(fullday)-1)) {
    if(i.day < (length(fullday)-1)) {
      df_1d = as.data.frame(colmean[fullday[i.day]:(fullday[i.day+1]-1)])
    } else {
      df_1d = as.data.frame(colmean[fullday[i.day]:fullday[i.day+1]]) # add 10th value
    }
    colnames(df_1d) = paste0("day_", (i.day-1), "-", (i.day))
    daily_mean = colMeans(df_1d)
    san.check = c(san.check, df_1d) # sanity check
    day_mean_dis = c(day_mean_dis, daily_mean) # daily mean
  }
  return(day_mean_dis)
}
