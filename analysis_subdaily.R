...
for (i.tid in 1:length(tids)) {
  ...
  for (i in 1:length(fcst$res.list)) {
    ...
    for (i.sub in 1:length(obs_subdiv)) {
      ...
      obs_subdiv_temp$res.list = list(obs_subdiv_temp$res.list[[1]])
      ...
      fcst_subdiv_temp$res.list = list(fcst_subdiv_temp$res.list[[1]])
      ...
      # some plotting
      plot.title = paste0(fcst$res.list[[i]]$TargetID, "_", fcst$res.list[[i]]$GroupID, "_", fcst$res.list[[i]]$MethodID, "_", fcst$res.list[[i]]$InitYear, "-", fcst$res.list[[i]]$InitDayOfYear, "_ld:", i.sub)
