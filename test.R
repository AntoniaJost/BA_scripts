...
# first the obs
obs.rot.1d = sl.trajectory.remaptime(oldtime = obs$res.list[[1]]$data$DaysLeadTime, newtime = 1, # hier muesste i statt 1 stehen fuer dynamisch
...
# now the fcst
fcst.rot.1d = sl.trajectory.remaptime(oldtime = fcst$res.list[[1]]$data$DaysLeadTime, newtime = 1,
...
for (i in 1:length(wholeNumbers)) {
  ...
  # Index für das Ende des Teildatensatzes
  if (i < length(wholeNumbers)) {
    nextNumber <- wholeNumbers[i + 1]
    endIdx <- which(floor(daysLeadTime) < nextNumber)
  } else {
    endIdx <- length(daysLeadTime)
  }
  
  # Teildatensatz erstellen
  teilDatensatz <- fcst$res.list[[1]]$data[startIdx:endIdx, ]
  
  # Zusätzliche Filterung entsprechend der Präzisierung
  if (i > 1) {
    previousEndIdx <- endIdx[i - 1]
    teilDatensatz <- teilDatensatz[teilDatensatz$DaysLeadTime > wholeNumbers[i-1], ]
  }
